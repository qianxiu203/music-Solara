import {
  AUDIO_HOST_RULES as BASE_AUDIO_HOST_RULES,
  getDefaultApiBase,
  getApiBases,
  resolveUpstream,
  STABLE_SOURCES,
...
let AUDIO_HOST_RULES: AudioHostRule[] = BASE_AUDIO_HOST_RULES;
...
async function proxyApiRequest(
  ...
  AUDIO_HOST_RULES = appendHostRules(env);

  const resolved = resolveUpstream(url.searchParams.get("api"), apiBases);
  if ("error" in resolved) {
    return jsonError(400, resolved.error);
  }

  const apiUrl = new URL(resolved.api);
  forwardSearchParams(url.searchParams, apiUrl, ["target", "api"]);
...
export async function onRequest(context: { request: Request; env: Record<string, unknown> }): Promise<Response> {
  ...
  try {
    ...
    return await proxyApiRequest(url, request, env);
  } catch {

...
function sanitizeStoredSearchState(data, defaultSource = SOURCE_OPTIONS_FN()[0].value) {
...
const AUDIO_PROXY_HOST_PATTERNS_FN = () => {
  // 基础规则在 index.html 引用阶段尚不可用，用已知默认值；
  // 运行时若 /api/sources 返回的规则不同，则动态更新此数组。
  return [
    /(^|\.)kuwo\.cn$/i,
    /(^|\.)bilivideo\.com$/i,
    /(^|\.)bilibili\.com$/i,
...
let AUDIO_PROXY_HOST_PATTERNS = AUDIO_PROXY_HOST_PATTERNS_FN();

function refreshAudioProxyPatterns(hosts) {
  AUDIO_PROXY_HOST_PATTERNS = hosts.map((h) => {
    try { return new RegExp(`(^|\\.)${h.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`, "i"); }
    catch { return null; }
  }).filter(Boolean);
}
...
function shouldProxyAudio(hostname) {
  ...
  return AUDIO_PROXY_HOST_PATTERNS.some((pattern) => pattern && pattern.test(hostname));
}
...
function getBuiltInSource(value) {
  return SOURCE_OPTIONS_FN().find((opt) => opt.value === value) || null;
}

// 用户自定义源（localStorage 持久化，D1 远程同步）
const CUSTOM_SOURCES_KEY = "customSources.v1";

function loadCustomSources() {
  try {
    const raw = safeGetLocalStorage(CUSTOM_SOURCES_KEY);
    const parsed = parseJSON(raw, []);
    if (!Array.isArray(parsed)) return [];
    return parsed.filter(
      (s) => s && typeof s.id === "string" && typeof s.name === "string" && typeof s.apiBase === "string"
    );
  } catch {
    return [];
  }
...
function persistCustomSources(sources) {
  safeSetLocalStorage(CUSTOM_SOURCES_KEY, JSON.stringify(sources));
}

function getAllSources() {
  return [...SOURCE_OPTIONS_FN(), ...loadCustomSources()];
}

function findSource(value) {
  return getAllSources().find((s) => s.value === value) || null;
}

const EXPLORE_RADAR_SOURCES_FN = (() => {
  // 默认使用内置稳定源；自定义源不加入探索雷达（未知 API 格式可能不兼容）
  return () => SOURCE_OPTIONS_FN().filter((s) => s.stable).map((s) => s.value);
})();

function getExploreRadarSources() {
  return EXPLORE_RADAR_SOURCES_FN();
}

function pickRandomExploreSource() {
  const stableSources = getExploreRadarSources();
  if (!Array.isArray(stableSources) || stableSources.length === 0) return SOURCE_OPTIONS_FN()[0]?.value || "netease";
  const index = Math.floor(Math.random() * stableSources.length);
  return stableSources[index];
}

function updateExploreRadarSources() {
  // 使 getExploreRadarSources() 始终反映当前稳定内置源；不保留状态
}
...
const savedLastSearchState = (() => {
    ...
    return sanitizeStoredSearchState(parsed, savedSearchSource || SOURCE_OPTIONS_FN()[0].value);
})();
...
function getApiBaseForSource(source) {
  // 内置源用代理默认上游（即 GD）；自定义源用自己的 apiBase
  const custom = loadCustomSources().find(s => s.value === source);
  if (custom && typeof custom.apiBase === "string") return custom.apiBase;
  const builtIn = getBuiltInSource(source);
...
const API = {
    ...
    resolveApi: (source) => getApiBaseForSource(source),
    ...
    search: async (keyword, source = "netease", count = 20, page = 1) => {
        const apiBase = API.resolveApi(source);
    ...
    getRadarPlaylist: async (playlistId = "3778678", options = {}, source = "netease") => {
        let count = 50;
        let pages = 1;
    ...
    getSongUrl: (song, quality = "320") => {
        const apiBase = API.resolveApi(song.source);
        const params = new URLSearchParams({
            types: "url",
            id: song.id,
...
function restoreStateFromSnapshot(snapshot) {
    const sanitized = sanitizeStoredSearchState(snapshot, state.searchSource || SOURCE_OPTIONS_FN()[0].value);
...
function buildSourceMenu() {
    ...
    const optionsHtml = sources.map(option => {
        const isActive = option.value === state.searchSource;
        const isCustom = typeof option.apiBase === "string";
        return `
            <div class="source-option${isActive ? " active" : ""}" data-source="${option.value}" role="option" aria-selected="${isActive}">
                ...
                ${isCustom ? '<i class="fas fa-puzzle-piece source-custom-badge" aria-hidden="true" title="自定义音源"></i>' : ""}
            </div>
        `;
    }).join("");
    const separator = `<div class="source-menu-separator" role="separator"></div>`;
    const manageHtml = `
        <div class="source-option source-manage-option" id="sourceManageOption" role="option">
            <i class="fas fa-plus" aria-hidden="true"></i>
            <span>${window.t ? window.t("管理音源") : "管理音源"}</span>
        </div>
    `;
    dom.sourceMenu.innerHTML = optionsHtml + separator + manageHtml;
    if (state.sourceMenuOpen) {
        scheduleSourceMenuPositionUpdate();
    }
    // 绑定管理入口
    dom.sourceMenu.querySelector("#sourceManageOption")?.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        closeSourceMenu();
    ...
    // 绑定自定义源删除按钮（事件委托）
    dom.sourceMenu.querySelectorAll(".source-delete-btn").forEach(btn => {
        btn.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            const customId = btn.dataset.id;
            if (customId) removeCustomSource(customId);
        });
    });
}

function updateSourceLabel() {
    const option = getAllSources().find(item => item.value === state.searchSource) || getAllSources()[0];
    ...
    dom.sourceSelectButton.setAttribute("aria-label", (window.t ? window.t("当前音源") : "当前音源") + "：" + option.label + "，" + (window.t ? window.t("点击切换音源") : "点击切换音源"));
    dom.sourceSelectButton.setAttribute("title", (window.t ? window.t("音源") : "音源") + "：" + option.label);
}
...
// ── 自定义源管理 ──────────────────────────────────────────
function openSourceManagerDialog() {
    let dialog = document.getElementById("sourceManagerDialog");
    if (!dialog) {
        dialog = document.createElement("div");
        dialog.id = "sourceManagerDialog";
        dialog.className = "source-manager-dialog";
        dialog.setAttribute("role", "dialog");
        dialog.setAttribute("aria-modal", "true");
        dialog.setAttribute("aria-label", window.t ? window.t("管理音源") : "管理音源");
        document.body.appendChild(dialog);

        const backdrop = document.createElement("div");
        backdrop.className = "source-manager-backdrop";
        dialog.appendChild(backdrop);

        const panel = document.createElement("div");
        panel.className = "source-manager-panel";
        dialog.appendChild(panel);

        const header = document.createElement("div");
        header.className = "source-manager-header";
        header.innerHTML = `<h3>${window.t ? window.t("管理音源") : "管理音源"}</h3>
            <button class="source-manager-close" type="button" aria-label="${window.t ? window.t("关闭") : "关闭"}">
                <i class="fas fa-times" aria-hidden="true"></i>
            </button>`;
        panel.appendChild(header);

        const body = document.createElement("div");
        body.className = "source-manager-body";
        panel.appendChild(body);

        const customList = document.createElement("div");
        customList.className = "source-manager-list";
        customList.id = "sourceManagerList";
        body.appendChild(customList);

        const separator = document.createElement("div");
        separator.className = "source-manager-separator";
        body.appendChild(separator);

        const formTitle = document.createElement("p");
        formTitle.className = "source-manager-form-title";
        formTitle.textContent = window.t ? window.t("添加自定义音源") : "添加自定义音源";
        body.appendChild(formTitle);

        const form = document.createElement("form");
        form.className = "source-manager-form";
        form.id = "sourceManagerForm";
        ...
        nameGroup.innerHTML = `
            <label class="field-label" for="customSourceName">${window.t ? window.t("音源名称") : "音源名称"}</label>
            <div class="input-wrapper">
                <i class="fa-solid fa-tag" aria-hidden="true"></i>
                <input type="text" id="customSourceName" placeholder="${window.t ? window.t("例：我的源") : "例：我的源"}"
                    autocomplete="off" autocapitalize="off" spellcheck="false" required maxlength="40">
            </div>`;
        form.appendChild(nameGroup);

        // 源 ID
        const idGroup = document.createElement("div");
        idGroup.className = "field";
        idGroup.innerHTML = `
            <label class="field-label" for="customSourceId">${window.t ? window.t("源 ID") : "源 ID"}</label>
            <div class="input-wrapper">
                <i class="fa-solid fa-fingerprint" aria-hidden="true"></i>
                <input type="text" id="customSourceId" placeholder="${window.t ? window.t("例：netease / my_api") : "例：netease / my_api"}"
                    autocomplete="off" autocapitalize="off" spellcheck="false" required maxlength="60">
            </div>`;
        form.appendChild(idGroup);

        // API 地址（下拉白名单）
        const apiGroup = document.createElement("div");
        apiGroup.className = "field";
        apiGroup.innerHTML = `
            <label class="field-label" for="customSourceApi">${window.t ? window.t("API 服务") : "API 服务"} <span class="field-hint">(${window.t ? window.t("留空=默认 GD") : "留空=默认 GD"})</span></label>
            <div class="input-wrapper">
                <i class="fa-solid fa-globe" aria-hidden="true"></i>
                <select id="customSourceApi" class="source-manager-select">
                    <option value="">${window.t ? window.t("使用默认（GD Studio）") : "使用默认（GD Studio）"}</option>
                </select>
            </div>`;
        ...
        submitBtn.className = "source-manager-submit-btn";
        submitBtn.textContent = window.t ? window.t("添加音源") : "添加音源";
        form.appendChild(submitBtn);

        // 关闭按钮
        header.querySelector(".source-manager-close").addEventListener("click", closeSourceManagerDialog);
        backdrop.addEventListener("click", closeSourceManagerDialog);
        document.addEventListener("keydown", function sourceMgrEsc(e) {
            if (e.key === "Escape") {
                closeSourceManagerDialog();
                document.removeEventListener("keydown", sourceMgrEsc);
            }
        });

        // 提交
        form.addEventListener("submit", handleSourceManagerFormSubmit);

        // 填充 API 下拉（取 /api/sources 数据）
        populateApiSelect();
    }
...
function populateApiSelect() {
    const select = document.getElementById("customSourceApi");
    if (!select) return;
    // 这里使用内置默认值；若将来从 /api/sources 拉取，可在此处替换
    select.innerHTML = `<option value="">${window.t ? window.t("使用默认（GD Studio）") : "使用默认（GD Studio）"}</option>`;
}
...
function renderSourceManagerList() {
    ...
    if (custom.length === 0) {
        list.innerHTML = `<p class="source-manager-empty">${window.t ? window.t("暂无自定义音源") : "暂无自定义音源"}</p>`;
        return;
    }
    list.innerHTML = custom.map(s => `
        <div class="source-manager-item" data-id="${s.id}">
            <div class="source-manager-item-info">
                <span class="source-manager-item-name">${escapeHtml(s.name)}</span>
                <span class="source-manager-item-id">${escapeHtml(s.id)}</span>
            </div>
            <button class="source-manager-item-delete" type="button" data-id="${s.id}" aria-label="${window.t ? window.t("删除") : "删除"} ${escapeHtml(s.name)}">
                <i class="fas fa-trash-alt" aria-hidden="true"></i>
            </button>
        </div>
    `).join("");
    list.querySelectorAll(".source-manager-item-delete").forEach(btn => {
...
function closeSourceManagerDialog() {
    const dialog = document.getElementById("sourceManagerDialog");
    if (dialog) dialog.classList.remove("show");
    document.body.style.overflow = "";
}

async function handleSourceManagerFormSubmit(event) {
    event.preventDefault();
    const nameInput = document.getElementById("customSourceName");
    const idInput = document.getElementById("customSourceId");
    const apiSelect = document.getElementById("customSourceApi");
    if (!nameInput || !idInput || !apiSelect) return;

    const name = (nameInput.value || "").trim();
    const id = (idInput.value || "").trim().replace(/\s+/g, "_");
    let apiBase = (apiSelect.value || "").trim();

    if (!name || !id) {
        showNotification(window.t ? window.t("请填写名称和源 ID") : "请填写名称和源 ID", "error");
        return;
    }
    const idPattern = /^[a-zA-Z0-9_\-]+$/;
    if (!idPattern.test(id)) {
        showNotification(window.t ? window.t("源 ID 仅支持字母、数字、下划线和横杠") : "源 ID 仅支持字母、数字、下划线和横杠", "error");
        return;
    }

    const current = loadCustomSources();
    if (current.some(s => s.value === id)) {
        showNotification(window.t ? window.t("该源 ID 已存在，请勿重复添加") : "该源 ID 已存在，请勿重复添加", "error");
        return;
    }
    ...
    persistCustomSources([...current, newEntry]);
    showNotification(`${window.t ? window.t("已添加") : "已添加"}：${name}`, "success");
    const formEl = document.getElementById("sourceManagerForm");
...
function removeCustomSource(customId) {
    const current = loadCustomSources();
    const filtered = current.filter(s => s.id !== customId);
    if (filtered.length === current.length) return;
    persistCustomSources(filtered);
    const removed = current.find(s => s.id === customId);
    if (state.searchSource === (removed?.value || "")) {
        state.searchSource = SOURCE_OPTIONS_FN()[0].value;
        safeSetLocalStorage("searchSource", state.searchSource);
        updateSourceLabel();
    }
    buildSourceMenu();
    showNotification(`${window.t ? window.t("已删除") : "已删除"}：${removed?.label || ""}`, "success");
}
...
async function handleLogout() {
    try {
        ...
        showNotification(window.t ? window.t("退出失败，请手动清除 cookie") : "退出失败，请手动清除 cookie", "error");
    }
}
